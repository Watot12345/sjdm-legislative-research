...
<main class="p-8">
    ...
    <div class="bg-white rounded-xl shadow mb-8 no-print border border-slate-200">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-5 border-b gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Recent Module Activity</h2>
                <p class="text-slate-500 mt-0.5 text-xs">Latest activity logs and audit trails across all modules</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left" id="activityTable">
                        ...
                        <tr class="hover:bg-slate-50 transition activity-row">
                            <td class="p-4 font-medium">
                                <span class="px-2.5 py-1 rounded-full text-xs font-semibold 
                                    <?php echo $row['module'] == 'Impact Assessment' ? 'bg-purple-100 text-purple-700' : 
                                              ($row['module'] == 'Data Collection' ? 'bg-blue-100 text-blue-700' : 
                                              ($row['module'] == 'Benchmarking' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700')); ?>">
                                    <?php echo htmlspecialchars($row['module']); ?>
                                </span>
                            </td>
                            <td class="p-4 text-xs font-medium text-slate-800"><?php echo htmlspecialchars($row['action']); ?></td>
                            <td class="p-4 text-xs text-slate-600 font-mono"><?php echo htmlspecialchars($row['user']); ?></td>
                            <td class="p-4 text-xs text-slate-500"><?php echo date('M j, Y h:i A', strtotime($row['timestamp'])); ?></td>
